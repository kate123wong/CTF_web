...
| 2021.7.18～2021.7.19       | 完成exp、check脚本编写；web docker构建完成。                 | 是           |
...
| --------- | -------- | ----------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| /         | GET/POST | 无                      | login.html页面                                               |                                                              |
| /login    | GET      | 无                      | login.html页面                                               |                                                              |
| /login    | POST     | username passwd         | status session                                               | status可取：201、205、206、207、210。session是后端由时间戳生成的字符串。 |
| /register | GET      | 无                      | register.html页面                                            |                                                              |
| /register | POST     | username passwd         | status                                                       | status可取：202、203、204、210                               |
| /index    | GET      | username passwd session | 成功：index.html页面；失败：“You  have not login in.”        | 该页面是首页，只有用户已经登录，也就是拿到session，才能返回该页面 |
| /user     | GET      | username passwd session | 成功&admin：admin/index.html页面；成功&普通用户：user/index.html;失败："You have not login in." |                                                              |
| /friends  | POST     | 无                      | {“status”:208,”friends”: friends}                            | friends是一个username的字典。key为usernamei,value是username值。 |
| /myself   | POST     | username passwd session | {“status”:status,”data”: data}                               | status可取：209、210（没有data）；208（此时data为数据库查询的Users表的返回内容） |
